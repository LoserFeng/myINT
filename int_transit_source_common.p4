...
control Int_metadata_insert(inout headers hdr, in int_metadata_t int_metadata,inout standard_metadata_t standard_metadata)    //这里就是往INT里面加各种各样的数据,并且不同的命令有不同的组合。
{
        ...
        action int_set_header_2() { // hop latency  跳转延迟
...
control Int_outer_encap(inout headers hdr, in int_metadata_t int_metadata)    //encap是封装的意思
{
...
inout standard_metadata_t standard_metadata)
{
        ...
        Int_metadata_insert() int_metadata_insert;   //这个是不是声明了之前定义过的方法。。？
        Int_outer_encap() int_outer_encap;
        action int_hop_cnt_decrement() {
            hdr.int_header.remaining_hop_cnt = hdr.int_header.remaining_hop_cnt - 1;//hop数量减少
        }
        ...
        apply{
            if(hdr.int_header.isValid()) {
                if(hdr.int_header.remaining_hop_cnt != 0 && hdr.int_header.e == 0) {
                    ...
                    int_hop_cnt_exceeded();           //如果没有hop_cnt剩余，说明INT已经存的数据满了，因此做一个标记，hdr.int_header.e
                }
            }
        }
}
